...
class Payroll extends BaseController {
    ...
    public function index() {
        if (session('logged_in')) {
            ...
            helper('assets');
            $data = array( 'user_role' => $session_array['user_role'],'title'=>'Payroll Register','metakeys'=>'','metadesc'=>'',
                            'stylesheet'=>$this->get_stylesheets(),'javascripts'=>$this->get_javascripts(),
                            'module'=>'','salary_list'=>model('Payroll_model')->get_all_lists());
            echo view($this->layout, $data);
            echo view('list/salary', $data);
    ...
    function suspense_add_submit() {
        $rules=['search_box'=> 'trim|required','amount'=>'trim|required'];
        if (!$this->validate($rules)) {
